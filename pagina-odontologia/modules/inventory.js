...
class InventoryModule {
  ...
  renderEmptyState() {
    return `
      <div class="empty-state">
        ...
        <button class="btn btn-primary mt-2" onclick="inventoryModule.showAddForm()">
          ➕ Agregar Producto
        </button>
      </div>
    `;
  ...
  renderTable(inventory) {
    return `
      ...
      return `
                <tr ${isLowStock ? 'style="background: #FFF9E6;"' : ''}>
                  <td><strong>${item.id}</strong></td>
                  <td>${item.name}</td>
                  <td>${item.category}</td>
                  <td>
                    <strong style="color: ${isLowStock ? 'var(--warning)' : 'var(--success)'};">
          ...
          : '<span class="badge badge-success">✓ Normal</span>'
                  ...
                  <td>
                    <div class="action-buttons">
                      <button class="action-btn" style="background: var(--success);" onclick="inventoryModule.showStockAdjustment('${item.id}')" title="Ajustar Stock">
                        📊
                      </button>
                      <button class="action-btn action-btn-view" onclick="inventoryModule.viewHistory('${item.id}')" title="Ver Historial">
                        📜
                      </button>
                      <button class="action-btn action-btn-edit" onclick="inventoryModule.showEditForm('${item.id}')" title="Editar">
                        ✏️
                      </button>
                      <button class="action-btn action-btn-delete" onclick="inventoryModule.deleteItem('${item.id}')" title="Eliminar">
                        🗑️
                      </button>
                    </div>
                  </td>
  ...
  renderStats() {
    ...
    const lowStockCount = db.getLowStockItems().length;
    ...
    return `
      <div class="stats-grid">
        ...
        <div class="stat-card">
          <div class="stat-icon" style="background: linear-gradient(135deg, #F6AD55 0%, #ED8936 100%);">⚠️</div>
          <div class="stat-content">
            <h3>${lowStockCount}</h3>
        ...
        <div class="stat-card">
          <div class="stat-icon" style="background: linear-gradient(135deg, #4299E1 0%, #3182CE 100%);">🏷️</div>
          <div class="stat-content">
            <h3>${categories}</h3>
  ...
  updateTable() {
    ...
    if (container) {
      ...
      container.innerHTML = inventory.length === 0 ? this.renderEmptyState() : this.renderTable(inventory);
    }
  }
  ...
  showLowStockItems() {
    ...
    modal.create('⚠️ Productos con Stock Bajo', content, [
  ...
  showAddForm() {
    ...
    modal.form('Nuevo Producto', [
      { name: 'name', label: 'Nombre del Producto', type: 'text', required: true, placeholder: 'Ej: Guantes de látex' },
      {
        name: 'category',
        label: 'Categoría',
        type: 'select',
      ...
      { name: 'minStock', label: 'Stock Mínimo', type: 'number', required: true, min: 0 },
  ...
  deleteItem(id) {
    ...
    modal.confirm(
      'Eliminar Producto',
      `¿Estás seguro de que deseas eliminar <strong>${item.name}</strong> del inventario?`,
      () => {
  ...
  showStockAdjustment(id) {
    ...
    const fields = [
      { type: 'header', label: '📊 Stock Actual' },
      {
        name: 'currentStock',
        label: 'Stock Actual',
        type: 'text',
      ...
      { type: 'header', label: '📝 Ajuste de Inventario' },
      {
        name: 'type',
    ...
    if (predictions) {
      fields.push({ type: 'header', label: '📈 Sugerencias Basadas en Historial' });
      fields.push({
        name: 'suggestion',
        label: 'Información',
        type: 'textarea',
        value: `Consumo mensual promedio: ${predictions.avgMonthlyUsage} ${item.unit}\nCantidad sugerida para reorden: ${predictions.suggestedReorderQuantity} ${item.unit}\nDías hasta reorden: ${predictions.daysUntilReorder > 0 ? predictions.daysUntilReorder : 'Reabastecer ahora'}`
      });
    }
  ...
  viewHistory(id) {
    ...
    const content = `
            <div>
                ...
                <h4 style="color: var(--primary-blue); margin-bottom: 1rem;">📜 Historial de Movimientos (${movements.length})</h4>
                                ...
                                <div class="card" style="padding: 1rem; margin-bottom: 0.75rem; border-left: 4px solid ${isIncrease ? 'var(--success)' : 'var(--danger)'};">
                                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
                                        ...
                                        <div style="text-align: right;">
                                            ...
                                            <small style="color: var(--text-secondary);">
                                                ${m.previousStock} → ${m.newStock} ${item.unit}
                                            </small>
                                        </div>
                                    </div>
  ...
  renderMonthlyReport(itemId, item) {
    ...
    return `
            <div style="margin-top: 1.5rem;">
                <h4 style="color: var(--primary-blue); margin-bottom: 1rem;">📊 Reporte Mensual - ${now.toLocaleDateString('es-MX', { month: 'long', year: 'numeric' })}</h4>
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem;">
